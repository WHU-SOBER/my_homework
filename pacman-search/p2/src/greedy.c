...
int len(int s_x, int s_y, int e_x, int e_y, game_state_t state)
{
	int dx[] = { -1, 0, 1, 0 };
	int dy[] = { 0, 1, 0, -1 };
	int pre[100][100];
...
char* ShowPath(game_state_t state, int s_x, int s_y, int e_x, int e_y)
{
	int dx[] = { -1, 0, 1, 0 };
	int dy[] = { 0, 1, 0, -1 };
	int pre[100][100];
...
int main() {
	...
	greedy(state);
	char * re = ShowPath(state, state.start_x, state.start_y, x[ans[0]], y[ans[0]]);
	printf("%s", re);
	free(re);
	for (int i = 1; i < cnt; i++)
	{
		re = ShowPath(state, x[ans[i - 1]], y[ans[i - 1]], x[ans[i]], y[ans[i]]);
		printf("%s", re);
		free(re);
	}
	// 
