...
int next_frame(game_t *cur_game){
	...
	update_snake_head(cur_game->snake1,cur_game->board,cur_game->growth_per_food);
	update_snake_head(cur_game->snake2,cur_game->board,cur_game->growth_per_food);
	update_snake_tail(cur_game->snake1,cur_game->board);
	update_snake_tail(cur_game->snake2,cur_game->board);
	if((cur_game->board->cells[cur_game->snake1->head->row*cur_game->board->cols+cur_game->snake1->head->col] == CELL_WALL ||cur_game->board->cells[cur_game->snake1->head->row*cur_game->board->cols+cur_game->snake1->head->col] == CELL_SNAKE)||(cur_game->board->cells[cur_game->snake2->head->row*cur_game->board->cols+cur_game->snake2->head->col] == CELL_WALL ||cur_game->board->cells[cur_game->snake2->head->row*cur_game->board->cols+cur_game->snake2->head->col] == CELL_SNAKE))
    		return 1;
	cur_game->board->cells[cur_game->snake1->head->row*cur_game->board->cols+cur_game->snake1->head->col] = CELL_SNAKE;
	cur_game->board->cells[cur_game->snake2->head->row*cur_game->board->cols+cur_game->snake2->head->col] = CELL_SNAKE;
	return 0;
